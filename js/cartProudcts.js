...
let proudectInCart = localStorage.getItem("proudectInCart"); // المنتجات الموجودة في السلة
let allProducts = document.querySelector(".products"); // قسم عرض كل المنتجات في السلة
let allfavorites = document.querySelector(".favorites"); // قسم عرض المفضلة
let totalPrice = document.querySelector(".total .totalPrice"); // عنصر عرض السعر الكلي

// إذا كان هناك منتجات في السلة، ارسمها
if (proudectInCart) {
...
// دالة لرسم المنتجات في السلة
function drawProudectCart(products) {
  let productCart = products.map((item) => {
    let quantity = +(localStorage.getItem(`quantity-${item.id}`)) || 1; // جلب الكمية أو تعيين 1
  ...
  allProducts.innerHTML = productCart.join(''); // عرض كل المنتجات في السلة
}

// -----------------------------------------------------------------------------
// جلب المنتجات من localStorage أو تعيين مصفوفة فارغة
...
// حساب السعر الكلي إذا كانت هناك منتجات
if (addItemStorage) {
  addItemStorage.map((item) => {
    total += +item.salePrice * +(localStorage.getItem(`quantity-${item.id}`));
  });
  totalPrice.innerHTML = total / 2; // عرض السعر الكلي
}

// -----------------------------------------------------------------------------
// دالة إزالة منتج من السلة
function removeFromCart(id) {
  let itemIndex = addItemStorage.findIndex((item) => item.id === id); // إيجاد مكان المنتج في المصفوفة
  let quantityElement = document.getElementById(`quantity-${id}`);
  let quantity = +(quantityElement.innerHTML);

  if (itemIndex !== -1) {
    addItemStorage.splice(itemIndex, 1); // إزالة المنتج من المصفوفة
    savestroge("proudectInCart",addItemStorage); // تحديث localStorage
    total = 0;

    let productItem = document.getElementById(`product-${id}`);
    if (productItem) {
      productItem.remove(); // إزالة المنتج من الصفحة
    }
...
function pls(id, salePrice) {
  ...
  localStorage.setItem(`quantity-${id}`, quantity.toString()); // تحديث الكمية في localStorage

  total += (+salePrice); // تحديث السعر الكلي
  totalPrice.innerHTML = total;
  savestroge("totalPrice",total);
}
...
function mins(id, salePrice) {
  ...
  if (quantity > 1) {
    ...
    removeFromCart(id); // إزالة المنتج إذا كانت الكمية صفر
  }
}

// -----------------------------------------------------------------------------
// قائمة المنتجات (يمكن تعديلها أو جلبها من قاعدة بيانات)
let products = [
...
function drawFavData() {
  let favorites = JSON.parse(localStorage.getItem('favorites')) || []; // جلب المفضلة من localStorage

  let pro = [];
  let indicators = '';
  let slideContent = '';
  const itemsPerSlide = 3; // عدد العناصر في كل شريحة

  for (let i = 0; i < favorites.length; i++) {
    const favoriteId = favorites[i];
    const item = products.find((product) => product.id === favoriteId); // البحث عن بيانات المنتج
    if (item) {
      const heartIconClass = 'fas'; // أيقونة قلب ممتلئ
  ...
  carouselInner.innerHTML = pro.join(''); // عرض شرائح المنتجات

  const carouselIndicators = document.querySelector('.carousel-indicators');
  carouselIndicators.innerHTML = indicators; // عرض المؤشرات
}
...
function removeFromFavorites(id) {
  ...
  heartIcon.classList.remove("fas"); // قلب ممتلئ
  heartIcon.classList.add("far"); // قلب فارغ
  ...
  savestroge('favorites',favorites); // تحديث localStorage
  drawFavData(); // إعادة رسم المفضلة
}
